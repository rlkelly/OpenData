...
third_weights = 2*np.random.random((3,2)) - 1
...
def secret_formula(lst):
    return ((lst[0]*lst[3]+lst[4])/lst[2] > 1.3).conjugate(), (sum(lst) < 2).conjugate()

# Take 100,000 batches of 100
for batch in xrange(500000):
    temp = [[random.random() for _ in range(5)] for _ in range(1)]
    
    X = np.array(temp)
    y = np.array( [secret_formula(row) for row in X] )
    ...
    if (batch % 10000) == 0 and batch > 0:
        ...
        print("Error at iteration {BATCH}:\t \t {ERROR}".format(BATCH=batch, ERROR=mean_error))
